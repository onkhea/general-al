...
//         MessageText := StrSubstNo(MessageLabel, Rec."Sender ID", COMPANYNAME, GetType(Rec), Https);
//         TelegramUsers.Reset();
//         TelegramUsers.SetRange("Approval User ID", Rec."Sender ID");
//         case Rec."Table ID" of
//             36:
//                 if TelegramUsers.Get(2) then begin
//                     Telegram.SendMessage(TelegramUsers."Telegram Chat ID", MessageText, TelegramUsers.id);
//                 end else begin
//                     Message('The message to Telegram  %1 \ could not be sent, the user was not found in the Telegram Setup table.', MessageText);
//                 end;
//             38:
//                 if TelegramUsers.Get(1) then begin
//                     Telegram.SendMessage(TelegramUsers."Telegram Chat ID", MessageText, TelegramUsers.Id);
//                 end else begin
//                     Message('The message to Telegram  %1 \ could not be sent, the user was not found in the Telegram Setup table.', MessageText);
//                 end;
...
//                     repeat
//                         if TelegramUsers.Get(3) then begin
//                             Telegram.SendMessage(TelegramUsers."Telegram Chat ID", MessageText, TelegramUsers.Id);
//                         end else begin
//                             Message('The message to Telegram  %1 \ could not be sent, the user was not found in the Telegram Setup table.', MessageText);
//                         end;
//                         if TelegramUsers.Get(4) then begin
//                             Telegram.SendMessage(TelegramUsers."Telegram Chat ID", MessageText, TelegramUsers.Id);
//                         end else begin
//                             Message('The message to Telegram  %1 \ could not be sent, the user was not found in the Telegram Setup table.', MessageText);
//                         end;
