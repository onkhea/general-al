codeunit 50655 "Telegram Event"
{
    ...
    begin
        ...
        if WorkGroup.find('-') then begin
            WorkGroup.SetRange("Table ID", Rec."Table ID");
            WorkGroup.SetRange(Code,TelegramUsers."Workflow User Group Code");
            repeat
                Telegram.SendMessage(TelegramUsers."Telegram Chat ID", MessageText, WorkGroup.Code);
    ...
    begin
        ...
        case ApprovalEntry."Table ID" of
            ...
            38:
                if TelegramUsers.Find('3') then begin
                    repeat
                        Telegram.SendMessageApp(TelegramUsers."Telegram Chat ID", MessageText, TelegramUsers."Workflow User Group Code");
                    until TelegramUsers.Next() = 0;
                end else begin
